...
void from_json(const nlohmann::json& j, content_type_t_& c) {
    try {
        ...
        if (source == "apache") {
            ...
            c.source = content_type_t_::source_t::NGINX;
...
static const std::regex EXTRACT_TYPE_REGEXP{R"(^\s*([^;\s]*)(?:;|\s|$))",
...
std::string ext_from_filename_(std::string filename) {
    ...
    if (dot_pos == std::string::npos)
        return ""; // no extension on names with a . like foobar
...
void init(void) {
    ...
    std::vector<private_::content_type_t_::source_t> preference{
            private_::content_type_t_::source_t::NGINX, private_::content_type_t_::source_t::APACHE,
    ...
    for (const auto& kv : private_::db_) {
        ...
        for (const auto& extension : exts) {
            if (private_::types_.count(extension)) {
                auto from = std::find(std::begin(preference), std::end(preference),
                ...
                if (private_::types_.at(extension) != "application/octet-stream" &&
                    (from > to || (from == to && private_::types_.at(extension).substr(0, 12) ==
                                                         "application/"))) {
                    // skip the remapping
                    continue;
                }
