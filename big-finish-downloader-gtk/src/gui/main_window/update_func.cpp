...
int libbf::gui::main_window::update_func(void * d) {
  ...
  gtk_progress_bar_set_fraction((GtkProgressBar *) m->progress_bar,
                                m->download_progress);
  gtk_label_set_text((GtkLabel *) m->downloading_status_ii,
                     m->status_ii.c_str());

  if (!m->items_fut.valid() && m->items.size() == 0)
    m->items_fut = std::async(
        std::launch::async, &libbf::gui::main_window::get_items, m, m->cookie);
  if (m->items_fut.valid()) {
    if (m->items_fut.wait_for(std::chrono::seconds(0)) ==
  ...
  if (!m->downloader.valid()) {
    ...
    if (gtk_tree_model_get_iter_first(a, &iter)) {
      ...
      do {
        ...
        if (y) {
          auto item =
              std::find_if(m->items.begin(), m->items.end(),
          ...
          m->downloader =
              std::async(std::launch::async, &libbf::gui::main_window::download,
                         m, item->first, std::move(m->quit.get_future()));
  ...
  } else if (m->downloader.wait_for(std::chrono::seconds(0)) ==
             std::future_status::ready) {
