...
libbf::gui::main_window::main_window(libbf::login_cookie c): cookie(c) {
  ...
  gtk_label_set_text((GtkLabel*)status_bar,("You are logged in as: "+cookie.get_email()).c_str() );
  
  title_bar = status_bar = GTK_WIDGET(gtk_builder_get_object(builder, "title_bar"));
