...
libbf::download create_download(lxb_dom_element_t* e) {
    ...
    std::string img_src(
            (char*) lxb_dom_element_get_attribute(img, (const lxb_char_t*) "src", 3, &len));
    std::string title(
    ...
    for (int i = 0; i < lxb_dom_collection_length(sdownloads); i++) {
        ...
        if (url->node.first_child->next == nullptr)
            name = std::string(
                    (char*) ((lxb_dom_text*) url->node.first_child)->char_data.data.data);
        else {
            name = std::string((char*) ((lxb_dom_text*) url->node.first_child->next->first_child)
                                       ->char_data.data.data);
        }
        std::string href(
                (char*) lxb_dom_element_get_attribute(url, (const lxb_char_t*) "href", 4, &len));
        trim(name);
