...
libbf::download::get_downloads(libbf::login_cookie & cookie) {
  ...
  while (true) {
    cpr::Response r =
        cpr::Get(cpr::Url{"https://www.bigfinish.com" + url},
    ...
    status = lxb_html_document_parse(
        document, (const lxb_char_t *) r.text.c_str(), r.text.size());
    ...
    lxb_dom_collection_t * collection =
        lxb_dom_collection_make(&document->dom_document, 50);
    ...
    // Next Page
    {
      lxb_dom_collection_t * nextPage =
          lxb_dom_collection_make(&document->dom_document, 1);

      lxb_dom_elements_by_class_name(lxb_dom_interface_element(document->body),
                                     nextPage, (const lxb_char_t *) "page_nav",
                                     8);
      ...
      if (lxb_dom_collection_length(nextPage) > 0) {
        ...
        std::string         u((char *) lxb_dom_element_get_attribute(
            (lxb_dom_element_t *) el->node.first_child,
            (const lxb_char_t *) "href", 4, &len));
        url = u;
